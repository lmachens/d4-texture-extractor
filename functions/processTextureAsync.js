...
async function processTextureInternal(key, data) {
  ...
  const outputFolder = options.outputpath
    ? path.resolve(options.outputpath)
    : path.resolve(`./${options.outputFormat}`);
  ...
  const texconvCommand = buildCommandLine(texconvCommandLine, [
    `"${ddsSourceFilePath}"`,
    "-ft png",
    "-f R8G8B8A8_UNORM",
    "-y",
  ...
  const width =
    data.dwMipMapLevelMin > 0
      ? data.dwWidth * Math.pow(2, data.dwMipMapLevelMin)
      : data.dwWidth;
  const height =
    data.dwMipMapLevelMin > 0
      ? data.dwHeight * Math.pow(2, data.dwMipMapLevelMin)
      : data.dwHeight;

  if (options.noslice) {
    const croppedFilePath = await cropImageAsync(
      pngFilePath,
      data.dwWidth,
      data.dwHeight,
    ...
    const sliceCoords = calculateSliceCoords(
      data.ptFrame,
      data.dwWidth,
      data.dwHeight
    );
    await sliceImageAsync(
      pngFilePath,
      outputFolder,
      sliceCoords,
      options.outputFormat
  ...
  // } else {
  //     croppedFilePath = await cropImageAsync(pngFilePath, width, height, options.outputFormat);
  // }
