...
async function main() {
  ...
  for (let i = 0; i < dataFiles.length; i++) {
    ...
    // Add wildcard to base patterns for matching (e.g., "zmap" matches "zmap_Frac_*")
    if (!options.filters.some(pattern => minimatch(key, `${pattern}*`))) {
      continue;
    }
...
program
  ...
  .option(
    "-g, --gameFolder <path>",
    'Path to Diablo IV folder, for example "C:\\Program Files (x86)\\Diablo IV\\"'
  )
  .option("-c, --concurrency <number>", "number of concurrent tasks")
  .option("-o, --outputformat <format>", "png, jpg or webp")
  .option(
    "-p, --outputpath <path>",
    "Full or relative path to output folder, default is './{outputformat}'"
  )
  .option(
    "-f, --filter <wildcard>",
    "Wildcard to filter files to process, for example '2DUI*', no need to include .json extension, default is '*'"
  )
  .option(
    "-nc, --nocrop",
    "Do not crop images to the size of the texture, useful for map textures"
  )
  .option("-ns, --noslice", "Do not slice the images, useful for map textures")
  .option(
    "-nsf, --noslicefolders",
    "Do not use slice folders, instead save slicers to the output folder, prefixed with the file name"
  );
